  ...
  return (
    <div className="min-h-screen bg-[#0b0d14] text-slate-100 font-sans flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── ダークサイドバー（デスクトップのみ） ── */}
        <aside className={`hidden md:flex flex-col bg-[#0d0f1a] border-r border-white/5 shrink-0 sticky top-0 h-screen z-40 transition-all duration-300 ${sidebarCollapsed ? "w-16" : "w-60"}`}>
          {/* ロゴ + トグルボタン */}
                ...
                {lists.length === 0 ? (
                  ...
                  <>
                    ...
                    <div className="flex gap-3 mb-6 overflow-x-auto pb-2">
                      {lists.map((l) => (
                        <div
                          ...
                          className={`flex-shrink-0 rounded-2xl px-5 py-4 min-w-[152px] text-left transition-all border group relative cursor-pointer ${
                            selectedListId === l.id
                              ? "bg-gradient-to-br from-violet-600 to-indigo-600 border-transparent shadow-lg shadow-violet-500/30 text-white"
                              : "bg-[#12141f] border-white/8 hover:border-violet-500/40 hover:bg-[#1a1d2e]"
                          }`}
                        >
                          {editingListId === l.id ? (
                            <input
                            ...
                            <>
                              <div
                                className={`text-sm font-bold mb-2 pr-10 leading-tight ${selectedListId === l.id ? "text-white" : "text-slate-200"}`}
                      ...
                      <button
                        onClick={() => setShowImport(true)}
                        className="flex-shrink-0 rounded-2xl px-5 py-4 border border-dashed border-white/12 hover:border-violet-500/50 text-slate-500 hover:text-violet-400 transition-all text-sm bg-transparent hover:bg-[#1a1d2e]"
                      >
                        ＋ 追加
                      </button>
                    </div>
                          ...
                          <div className="flex items-center gap-2.5">
                            <div className="relative flex-1">
                              ...
                              <input type="text" value={search} onChange={(e) => { setSearch(e.target.value); setSelectedIndex(null); }}
                                ...
                                className="w-full pl-10 pr-9 py-2.5 bg-[#12141f] border border-white/10 rounded-xl text-sm text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-violet-500/60 focus:ring-2 focus:ring-violet-500/20 transition-all" />
                              {search && (
                            ...
                            <button
                              ...
                              className="shrink-0 flex items-center gap-1.5 px-3.5 py-2.5 bg-[#12141f] hover:bg-[#1a1d2e] border border-white/10 hover:border-violet-500/40 text-slate-400 hover:text-violet-400 rounded-xl text-xs font-medium transition-all"
                            >
                            ...
                            <div className="flex items-center gap-2">
                                  ...
                                  <button key={f} onClick={() => setFilterResult(f)}
                                    className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all shrink-0 ${
                                      filterResult === f
                                        ...
                                        : "bg-white/5 text-slate-400 border border-white/8 hover:border-white/20 hover:text-slate-200"
                                    }`}>
                              ...
                              {assigneeOptions.length > 0 && filterBarOpen && (
                                <select
                                  ...
                                  className="shrink-0 text-xs bg-[#12141f] border border-white/10 rounded-lg px-2.5 py-1.5 text-slate-400 focus:outline-none focus:border-violet-500/50 transition-all cursor-pointer"
                                >
                              ...
                              <button
                                ...
                                className="shrink-0 flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-white/5 border border-white/8 text-slate-500 hover:text-slate-300 hover:bg-white/8 text-xs transition-all"
